...
function mutationPolicyInstruction(triageAuthority) {
  switch (triageAuthority) {
    ...
    case "triage-fields":
      return "You have a standing policy to triage untriaged items without asking, where untriaged means the item has no Status. Triage means setting every required field needed to make the item actionable: owner, Status, priority, and the requirements that select a playbook. The Workstream Issue association is optional and still requires explicit approval when inferred. Never leave an item owner agent and Status ready with empty requirements, because no runner can claim it. Leave every already-triaged item to an explicit approval.";
    default:
